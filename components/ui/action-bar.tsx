    ...
    <ActionBarContext.Provider value={contextValue}>
      {ReactDOM.createPortal(
        <RootPrimitive
          ...
          ref={composedRef}
          className={cn(
            "fixed z-50 rounded-2xl border border-outline-base-em bg-s-l0-d4 shadow-md outline-none",
...
function ActionBarSelection(props: ActionBarSelectionProps) {
  ...
  return (
    <SelectionPrimitive
      ...
      {...selectionProps}
      className={cn(
        "flex items-center gap-1 rounded-sm pr-2 text-foreground font-medium text-xs text-nowrap tabular-nums",
        className,
      )}
    >
      <Checkbox
...
function ActionBarClose(props: ActionBarCloseProps) {
  ...
  if (asChild) {
    const ClosePrimitive = SlotPrimitive.Slot;
    return (
      <ClosePrimitive
...
function ActionBarSeparator(props: ActionBarSeparatorProps) {
  ...
  return (
    <SeparatorPrimitive
      ...
      {...separatorProps}
      className={cn(
        "in-data-[slot=action-bar-selection]:ml-0.5 in-data-[slot=action-bar-selection]:h-4 in-data-[slot=action-bar-selection]:w-px bg-outline-low-em",
...
function ActionBarMoreAction({ children }: { children: React.ReactNode }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={(props) => (
          <Button
            ...
            variant="ghost"
            size="sm"
            data-slot="action-bar-more-action"
            aria-label="More actions"
          >
            <Icon name="moreAction" className="size-4 text-muted-foreground" />
            Actions
          </Button>
        )}
      />
